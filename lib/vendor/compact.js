/**
 * Ported verbatim from fast-jev-compaction src/compact.ts @
 * e3f262a7f4d42bd8dd32ced30d26176f7cb545b0 (MIT). See THIRD_PARTY_NOTICES.md.
 * Only TypeScript type syntax and import specifiers were changed.
 *
 * Batching, the two noul questions asked per call, the KEEP / DROP_RESULT /
 * DROP_CALL decision, the transcript rebuild, and the reduction ratio.
 *
 * NOTE ON THE DEFAULT THRESHOLD: `DEFAULT_OPTIONS.keepThreshold = 0.5` is the
 * upstream value and is kept here only so the port stays byte-comparable with
 * its source. The DSH plugin never relies on it — `lib/adapter.js` always passes
 * every option explicitly, and its own default is the more conservative 0.4.
 */
...
function truncatedResultText(text, isError, headChars) {
  ...
  // One wording change from upstream: the note names this plugin, because it is
  // the model reading it, not the upstream project. Tracked in THIRD_PARTY_NOTICES.md.
  return `${head}[context-curator dropped ${text.length - headChars} chars of this tool result${
    isError ? ' (error)' : ''
